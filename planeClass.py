...
# Construct the path to the "python" subfolder inside opensky-api
opensky_path = os.path.join(script_dir, "opensky-api", "python")
sys.path.insert(0, opensky_path)  # Ensure Python looks in the correct location
class Plane:
    import configparser
    ...
    def run_opens(self, ac_dict):
        ...
        try:
            ...
            if ac_dict.time_position is not None:
                self.last_pos_datetime = datetime.fromtimestamp(ac_dict.time_position, timezone.utc)
    ...
    def run_adsbx_v1(self, ac_dict):
        ...
        try:
            ...
            if self.on_ground:
                self.alt_ft = 0
            self.last_pos_datetime = datetime.fromtimestamp(int(ac_dict['postime'])/1000, timezone.utc)
        except ValueError as e:
    ...
    def __str__(self):
        ...
        # Ensure we don't try to access tzinfo if last_pos_datetime is None
        if self.last_pos_datetime is None:
            time_since_contact = "No Data Available"
        else:
            # Ensure timezone-awareness
            if self.last_pos_datetime.tzinfo is None:
                self.last_pos_datetime = self.last_pos_datetime.replace(tzinfo=timezone.utc)
        ...
        output = [
            ...
            [(Fore.CYAN + "Nav Modes" + Style.RESET_ALL), (Fore.LIGHTGREEN_EX + ', '.join(self.nav_modes) + Style.RESET_ALL)] if "nav_modes" in self.__dict__ and self.nav_modes is not None else None,
    ...
    def get_time_since(self, datetime_obj):
        if datetime_obj != None:
            time_since = datetime.now(timezone.utc) - datetime_obj
        else:
            time_since = None
        return time_since
    ...
    def run_check(self):
        ...
        if  self.last_pos_datetime is not None:
            ...
            print("⚠️ Warning: last_pos_datetime is None! Using failsafe timestamp.")
            time_since_contact = timedelta(days=999)  # Prevents accidental false triggers        
        ...
        if self.last_pos_datetime is None:
            print("Warning: last_pos_datetime is None, skipping time calculation.")
            return  # Prevents execution of further time-based logic
    ...
    #Takeoff and Land Notification
        if self.tookoff or self.landed:
            ...
            if self.config.getboolean('DISCORD', 'ENABLE'):
                ...
                thread_id = self.config.get('DISCORD', 'THREAD_ID') if self.config.has_option('DISCORD', 'THREAD_ID') and self.config.get('DISCORD', 'THREAD_ID').strip() != "" else None
                sendDis(message, self.config, role_id, thread_id, self.map_file_name)
            #Twitter
            if self.config.getboolean('TWITTER', 'ENABLE') and Plane.main_config.getboolean('TWITTER', 'ENABLE'):
                import tweepy
                try:
        ...
        if self.feeding:
            ...
            if self.last_emergency is not None and not self.emergency_already_triggered:
                time_since_org_emer = datetime.datetime.now(timezone.utc) - self.last_emergency[0]
            ...
            if self.last_track is not None:
                ...
                for trace in self.circle_history["traces"]:
                    ...
                    # Handle None values before converting to float
                    lat = trace[1] if trace[1] is not None else 0.0
                    lon = trace[2] if trace[2] is not None else 0.0

                    coords.append((float(lat), float(lon)))  # Now guaranteed to be a float
